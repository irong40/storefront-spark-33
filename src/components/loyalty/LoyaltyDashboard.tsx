  ...
  const progressToNextTier =
    member && nextTier
      ? (member.lifetime_points /
          (member.lifetime_points + nextTier.pointsNeeded)) *
        100
      : 100;
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <Badge className={TIER_COLORS[member?.tier || "bronze"]}>
                {TIER_ICONS[member?.tier || "bronze"]}{" "}
                {member?.tier?.toUpperCase() || "BRONZE"}
              </Badge>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    Expires{" "}
                    {format(new Date(redemption.expires_at), "MMM d, yyyy")}
                  </p>
                </div>
        ...
        <CardContent>
          {rewardsLoading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {rewards?.map((reward) => {
                const canRedeem =
                  (member?.points_balance || 0) >= reward.points_required;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {canRedeem
                        ? "Redeem"
                        : `Need ${reward.points_required - (member?.points_balance || 0)} more pts`}
                    </Button>
                  </div>
                );
              })}
