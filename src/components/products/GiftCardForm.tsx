...
import { CalendarIcon, Gift, Loader2, ShoppingCart, Plus, Minus } from 'lucide-react';
...
export function GiftCardForm({ productId }: GiftCardFormProps) {
    ...
    const [giftCards, setGiftCards] = useState<GiftCardItem[]>([createEmptyCard()]);
        ...
        try {
            ...
            for (const card of giftCards) {
                ...
                const giftCardData = {
                    ...
                    recipientEmail: card.recipientType === 'someone_else' ? card.recipientEmail : '',
                    recipientName: card.recipientType === 'someone_else' ? card.recipientName : '',
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            {giftCards.map((card, index) => (
                <div key={card.id} className="border rounded-2xl p-6 space-y-5 relative">
                    {/* Card Header */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Gift className="h-5 w-5 text-primary" />
                            <h3 className="font-semibold">Gift Card {giftCards.length > 1 ? `#${index + 1}` : ''}</h3>
                        </div>
                        {giftCards.length > 1 && (
                            <Button
                                type="button"
                    ...
                    {/* Recipient Type */}
                    <div>
                        <Label className="text-sm font-medium mb-3 block">Who is this gift card for?</Label>
                        <RadioGroup
                        ...
                        >
                            <div className="flex items-center space-x-2">
                                ...
                                <Label htmlFor={`myself-${card.id}`} className="cursor-pointer">Myself</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <RadioGroupItem value="someone_else" id={`someone-${card.id}`} />
                                <Label htmlFor={`someone-${card.id}`} className="cursor-pointer">For someone else</Label>
                            </div>
                        </RadioGroup>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => updateCard(card.id, { recipientEmail: e.target.value })}
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        ...
                                        {card.deliveryDate ? format(card.deliveryDate, "PPP") : "Select date"}
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-auto p-0" align="start">
                                    <Calendar
                                        ...
                                        onSelect={(date) => date && updateCard(card.id, { deliveryDate: date })}
                    ...
                    {/* Personal Message */}
                    <div className="space-y-2">
                        <Label htmlFor={`message-${card.id}`}>Personal Message (optional)</Label>
                        <Textarea
                            id={`message-${card.id}`}
