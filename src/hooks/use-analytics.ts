...
export function useRevenueStats(period: TimePeriod = "month") {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RevenueStats> => {
      ...
      const totalRevenue =
        currentOrders?.reduce((sum, o) => sum + Number(o.total), 0) || 0;
      const orderCount = currentOrders?.length || 0;
      const avgOrderValue = orderCount > 0 ? totalRevenue / orderCount : 0;

      const previousRevenue =
        previousOrders?.reduce((sum, o) => sum + Number(o.total), 0) || 0;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CustomerStats> => {
      ...
      orders.forEach((order) => {
        ...
        if (
          !customerFirstOrder[email] ||
          orderDate < customerFirstOrder[email]
        ) {
      ...
      const newCustomers = Object.entries(customerFirstOrder).filter(
        ([_, date]) => date >= start && date <= end,
      ).length;
      ...
      const previousNewCustomers = Object.entries(customerFirstOrder).filter(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RecentOrder[]> => {
      const { data, error } = await supabase
        .from("orders")
        .select(
          "id, order_number, customer_name, email, total, created_at, status",
        )
    ...
    queryFn: async () => {
      ...
      orderItems?.forEach((item) => {
        ...
        categoryMap[categoryName] =
          (categoryMap[categoryName] || 0) + Number(item.total);
      });
