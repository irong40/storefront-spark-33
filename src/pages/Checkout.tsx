...
import { useLoyaltyRedemptions, LoyaltyRedemption } from '@/hooks/use-loyalty';
...
export default function Checkout() {
  ...
  const createOrder = async (payment: PaymentResult | null) => {
    ...
    try {
      ...
      // Mark loyalty redemption as used (non-blocking)
      if (appliedReward?.redemptionId) {
        supabase
          ...
          .eq('id', appliedReward.redemptionId)
          .then(({ error }) => {
            if (error) {
              console.error('Failed to mark redemption as used:', error);
            }
          });
      }
      ...
      const message = error instanceof Error ? error.message : 'Payment handled but order creation failed. Please contact support.';
      toast({
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              {/* Show user's active redemptions if logged in */}
              {user && redemptions && redemptions.length > 0 && !appliedReward && !appliedGiftCard && (
                <div className="mt-4 p-4 bg-muted/50 rounded-xl">
                  <p className="text-sm font-medium mb-2">Your Available Rewards:</p>
                  <div className="space-y-2">
                    {redemptions.map((redemption: LoyaltyRedemption) => (
                      <button
                        ...
                        onClick={() => {
                          setAppliedReward({
                            ...
                            rewardType: redemption.reward?.reward_type || 'discount_fixed',
                            rewardValue: redemption.reward?.reward_value || 0,
                            rewardName: redemption.reward?.name || 'Reward',
                            minOrderAmount: redemption.reward?.min_order_amount || 0
                          });
                        }}
                        disabled={isSubmitting || paymentComplete}
                      ...
                      >
                        ...
                        <div className="text-sm text-muted-foreground">
                          Code: {redemption.code} • Expires {new Date(redemption.expires_at).toLocaleDateString()}
                        </div>
                      </button>
                    ))}
                  </div>
